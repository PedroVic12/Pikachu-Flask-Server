...
export const ChecklistWidget = ({ tasks = [] }) => {
  if (!tasks || tasks.length === 0) return <div className="flex items-center justify-center h-full text-gray-400">Nenhuma tarefa encontrada.</div>;
  return (
...
export const MarkdownViewerWidget = ({ markdownText }) => {
  const htmlContent = React.useMemo(() => renderMarkdownToHTML(markdownText), [markdownText]);
  return <div className="w-full h-full p-4 md:p-8 overflow-y-auto markdown-view" dangerouslySetInnerHTML={{ __html: htmlContent }}></div>;
};

export const MarkdownEditorWidget = ({ markdown, onChange }) => {
