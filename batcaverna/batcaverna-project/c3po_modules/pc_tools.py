...
def falar(texto):
    try:
        ...
        print(
            "Erro: 'mpg123' nao encontrado. Por favor, instale-o (ex: sudo apt-get install mpg123)."
        )
...
def abrir_aplicativos():
    ...
    for app in APLICATIVOS_PARA_ABRIR:
        try:
            ...
            print(
                f"Erro: Comando '{app}' nao encontrado. Verifique o nome do aplicativo."
            )
        except Exception as e:
...
def iniciar_projetos():
    ...
    for projeto in PROJETOS:
        ...
        if not os.path.isdir(projeto["caminho"]):
            print(
                f"AVISO: O diretorio '{projeto['caminho']}' nao existe. Pulando este projeto."
            )
            continue
        ...
        try:
            ...
            if porta:
                print(
                    f"'{projeto['nome']}' iniciado. Acesse em http://localhost:{porta}"
                )
            else:
