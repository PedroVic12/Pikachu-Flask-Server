...
// Componente de gráfico com Chart.js
const CategoryChart = ({ data }) => {
    ...
    useEffect(() => {
        ...
        chartRef.current = new Chart(ctx, {
            ...
            options: {
                ...
                plugins: {
                    tooltip: { callbacks: { label: (ctx) => `R$ ${formatBRL(ctx.raw)}` } },
                    legend: { display: false }
                },
                scales: {
                    x: { ticks: { callback: (val) => `R$ ${val >= 1000 ? (val / 1000).toFixed(0) + 'k' : val}`, color: '#a1a1aa' }, grid: { color: '#27272a' } },
    ...
    return React.createElement('canvas', { ref: canvasRef, style: { maxHeight: '300px', width: '100%' } });
};

// StatCard
const StatCard = ({ label, value, iconName, color }) => (
    React.createElement(Card, { elevation: 0, sx: { border: '1px solid #27272a', bgcolor: 'background.paper', height: '100%' } },
...
// InvoiceFormDialog (mesmo da resposta anterior, usando safeIcon)
...
// DashboardTab (com CategoryChart)
            ...
            React.createElement(Grid, { size: { xs: 12, md: 6 } },
                React.createElement(Card, { sx: { border: '1px solid #27272a', bgcolor: 'background.paper' } },
                    React.createElement(CardContent, null,
                        ...
                        chartData.length ? React.createElement(CategoryChart, { data: chartData }) : React.createElement(Box, { height: 220, display: "flex", alignItems: "center", justifyContent: "center" }, React.createElement(Typography, { variant: "body2", color: "text.secondary" }, "Sem dados"))
                    )
                )
            ),
            React.createElement(Grid, { size: { xs: 12, md: 6 } },
