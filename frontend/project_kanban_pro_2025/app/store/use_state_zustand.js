// src/store/useCrisisStore.js
import { create } from 'zustand';

// Exemplo Prático: Containers vs. Componentes + Zustand
// Aqui está o exemplo de como separar a lógica visual (Componente Burro) da lógica de negócios (Página Container), usando o Zustand para aquele contador que muda de cor.


export const useCrisisStore = create((set) => ({
    threatLevel: 0,
    increment: () => set((state) => ({ threatLevel: state.threatLevel + 1 })),
    decrement: () => set((state) => ({ threatLevel: Math.max(0, state.threatLevel - 1) })),
}));
